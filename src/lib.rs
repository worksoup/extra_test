...
use mirai_j4rs::mj_base::env::FromInstanceTrait;
...
impl ExtraTest {
    pub fn fetch(protocol: MiraiProtocol, version: String) -> Box<dyn FnOnce(&Jvm, &Instance, &Instance)> {
        Box::new(move |jvm: &Jvm, _builder: &Instance, _config: &Instance| {
    ...
    pub fn load_form_file(protocol: MiraiProtocol, file: String) -> Box<dyn FnOnce(&Jvm, &Instance, &Instance)> {
        Box::new(move |jvm: &Jvm, _builder: &Instance, _config: &Instance| {
            println!("extra test - tmp - load");
